...
export class ClasseService {
  ...
  get(refresh: boolean = false): Observable<Classe[]>{
    ...
    if(enseignantStorage){
      ...
      if(!refresh){
        const cache = this.globalService.tryRequestFromCache(constantes.requestCache.classesList);
        if(cache)
          return cache;
      }
  ...
  getClasse(nIDNiveau: number, nIDBranche: number, nIDClasse: number, refresh: boolean=false): Observable<Classe[]> {
    const url = `${this.uriClasse}/${nIDNiveau}/${nIDBranche}/${nIDClasse}`;
    if(!nIDNiveau && !nIDBranche && !nIDClasse && !refresh){
      const objtStore = this.globalService.tryRequestFromCache(constantes.requestCache.classesList);
    ...
    return this.globalService.setHttpRequest(url,"GET",{},Headers).pipe(
      tap(res => {
        ...
        if(!nIDNiveau && !nIDBranche && !nIDClasse)
          localStorage.setItem(constantes.requestCache.classesList, JSON.stringify(res));
      })
    )
  }
  ...
  getClassState(refresh: boolean=false): Observable<ClassSelected[]>{
    ...
    if(!refresh){
      const objStore = this.globalService.tryRequestFromCache(constantes.requestCache.classeWithStat);
      if(objStore)
        return objStore;
    }

    return this.globalService.setHttpRequest(this.ClasseStatistique, "GET", {},Headers).pipe(
      tap(res => {
        localStorage.setItem(constantes.requestCache.classeWithStat, JSON.stringify(res))
      })
    )
  }
  ...
  getClass(nIDNiveau: string, nIDBranche: string, nIDClasse: number): Observable<Classe[]> {
    const url = `${this.uriClasse}/${nIDNiveau}/${nIDBranche}/${nIDClasse}`;
    return this.globalService.setHttpRequest(url,"GET",{},Headers)
  }

  getOne(idclasse: number): Observable<Classe> {
    return this.globalService.setHttpRequest(this.uri + '/' + idclasse, "GET", {},Headers);
  }

  update(classe: Classe): Observable<Classe> {
    const BASE_PATH = `${this.uri}/${classe.IDCLASSES}`;
    return this.globalService.setHttpRequest(BASE_PATH, "PUT", classe, Headers).pipe(
      tap(data => {
  ...
  create(Classe: Classe): Observable<Classe> {
    ...
    return this.globalService.setHttpRequest(BASE_PATH, "POST", Classe,Headers).pipe(
      tap(data => {
        localStorage.removeItem(constantes.requestCache.classesList)
      })
    );
  ...
  changeClasse(nIDNouvelleClasse: number, nIDELEVE: number){
    return this.globalService.setHttpRequest(`ELEVE_Changement_Classe/${nIDELEVE}/${nIDNouvelleClasse}`, "PUT", {})
  }
