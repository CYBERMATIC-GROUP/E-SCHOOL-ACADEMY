...
export class HistoriqueVersementCaisseComponent {
  ...
  ngOnInit(): void {
    ...
    this.loadCodeJournaux(0);
    this.modePaiementList$.subscribe(data => {
      console.log(data)
    })
      ...
      (data) => {
        ...
        data.forEach(element => {
          if(element.IDCAISSE == this.agent.CaisseAssociee){
            ...
            this.Solde = this.globalService.formatPrix(Number(element.Solde), " ");
          }
        });
      },
      (error) => {
  ...
  view() {
    if (this.IDCAISSE && this.Date && this.DateFin) {
      ...
      ref.componentInstance.content = 'Veuillez sélectionner une caisse et spécifier les dates de début et de fin.';
    }
  }

  onChangeModePayement(event: any){
  ...
  printHistoryque(){
    ...
    this.historiqueCaisseService.printHistorique(this.IDCAISSE, this.Date, this.DateFin, this.modePaiement, this.codeJournal, this.codeJournauxSelected).pipe(
      tap(res => {
        console.log(res)
        this.globalService.printFile(res.body.Etat, 'Historique des versements de caisses');
      }),
      finalize(() => {
