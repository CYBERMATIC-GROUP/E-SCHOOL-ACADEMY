...
export class CoursComponent {
  ...
  cours$!: Observable<DevoirEnseignant[]>
  isLoading!:boolean
  ...
  getDevoisenseignant() {
    ...
    if(this.IDMATIERE, this.IDCLASSE){
      this.cours$ = this.enseigantService.getDevoirsEnseignants(this.IDMATIERE, this.IDCLASSE, this.enseignant.IDENSEIGNANT).pipe(
        tap(res => {
  ...
  createCours(){
    ...
    if(this.IDCLASSE && this.IDMATIERE){
      ...
      this.router.navigate(['/espace-enseignant/cours/form/' + actionOnForm.CREATE + '/0'], navigationExtras)
    }else{
