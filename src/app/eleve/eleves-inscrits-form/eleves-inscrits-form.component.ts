...
export class ElevesInscritsFormComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.nationaliteList$ = this.nationnaliteService.get();
      this.etatSanitairesList$ = this.etatSanitaireService.get();
      this.statutElevesList$ = this.statutService.get();
      ...
      if(this.action == "reinscrire")
        this.titlePage = "Réinscrire l'élève"
      else if(this.action == "edit")
        this.titlePage = "Modifier les informations de l'élève"
      else if(this.action == "view")
        this.titlePage = "Fiche de l'élève "
  ...
  initELeveInscrit(){
    ...
    const settingForm = (validatorsList: any[] | boolean = false) => {
      if(validatorsList == false)
        return [{value: null, disabled: this.action == 'view' ? true : false}]
      else if(validatorsList == true)
        return [{value: null, disabled: this.action == 'view' ? true : false}, Validators.required]

      return [{value: null, disabled: this.action == 'view' ? true : false}, validatorsList]
    }
    ...
    this.eleveInscritForm.get('SMS_ParentParDefaut')?.valueChanges.subscribe(value => {
  ...
  openTableClasse(){
    ...
    ref.afterClosed().subscribe(res => {
      this.eleveInscritForm.get('IDCLASSES')?.setValue(ref.componentInstance.classeSelected.IDCLASSES);
      this.classeChosed = ref.componentInstance.classeSelected;
      console.log(this.classeChosed)
      this.eleveInscritForm.get('textClasseValue')?.setValue(this.classeChosed.CodeClasse);
    });
  }
...
openStatusEleveDialog() {
  ...
  onInputNomOrPrenom(){
    if(!this.eleveInscritForm.value.IDCLASSES){
      this.globalService.alert("Veuillez d'abord sélectionner la classe", 'Attention !', "danger", "", "OK");
    }
  }
  ...
  onSubmitForm(){
    ...
    this.eleveSerivice.update(eleve).pipe(
      tap(res => {
        this.globalService.toastShow("Eleve " + eleve.Fr_Nom + " a été mis à jour avec succès !", "Modification");
        console.log(res);
  ...
  private setSmasValidators (value: number) {
    ...
    if(value == sms_parent.PERE){
      this.eleveInscritForm.get('Fr_NomPrenomPere')?.setValidators(Validators.required);
      this.eleveInscritForm.get('Fr_NomPrenomPere')?.updateValueAndValidity();
      this.eleveInscritForm.get('TelMobilePere')?.setValidators(Validators.required);
      this.eleveInscritForm.get('TelMobilePere')?.updateValueAndValidity();
    }
    else if(value == sms_parent.MERE){
      console.log('mere');
      this.eleveInscritForm.get('Fr_NomPrenomMere')?.setValidators(Validators.required);
      this.eleveInscritForm.get('Fr_NomPrenomMere')?.updateValueAndValidity();
      this.eleveInscritForm.get('TelMobileMere')?.setValidators(Validators.required);
      this.eleveInscritForm.get('TelMobileMere')?.updateValueAndValidity();
    }
    else if(value == sms_parent.TUTEUR){
      console.log('tuteur');
      this.eleveInscritForm.get('Fr_NomPrenomTuteur')?.setValidators(Validators.required);
      this.eleveInscritForm.get('Fr_NomPrenomTuteur')?.updateValueAndValidity();
      this.eleveInscritForm.get('TelMobileTuteur')?.setValidators(Validators.required);
      this.eleveInscritForm.get('TelMobileTuteur')?.updateValueAndValidity();
    }
    else if(value == sms_parent.ELEVE){
      console.log('eleve');
      this.eleveInscritForm.get('TelMobile')?.setValidators(Validators.required);
      this.eleveInscritForm.get('TelMobile')?.updateValueAndValidity();
    }
  }
  ...
  onChangeClass(){
    ...
    ref.afterClosed().subscribe(result => {
      if (ref.componentInstance.isModified){
        this.globalService.reloadComponent('eleve/inscription/edit/' + eleve.IDELEVE)
      }
    })
  }
  ...
  printDoc(typeimpression: number){
    ...
    this.eleveSerivice.impressionDocumentEleve(typeimpression, [{IDELEVE: this.IDEleve}]).pipe(
      tap(res => {
